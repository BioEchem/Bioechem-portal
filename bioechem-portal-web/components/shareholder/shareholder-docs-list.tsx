...
const CATEGORY_COLORS: Record<string, string> = {
  ...
  financial:  "bg-bio-green/10 text-bio-green",
...
export function ShareholderDocsList({ docs }: { docs: Doc[] }) {
  const [downloading, setDownloading] = useState<string | null>(null);

  // Group by category
  const categories = ["financial", "report", "governance", "meeting", "general"] as const;
    ...
    return (
      <PortalCard>
        <div className="flex flex-col items-center py-12 text-center">
          ...
          <p className="mt-1 text-xs text-bio-text-muted">Documents shared by BioEChem will appear here.</p>
        </div>
      </PortalCard>
    );
  }
