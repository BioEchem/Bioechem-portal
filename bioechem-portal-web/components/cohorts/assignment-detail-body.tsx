...
/**
 * Renders a single assignment's detail (instructions, submission form,
 * grading list) inline within the cohort page's Assignments tab, so the URL
 * stays `/cohorts/[id]?tab=assignments&assignmentId=...` like every other tab
 * instead of navigating to a separate route.
 */
export async function AssignmentDetailBody({
  cohortId,
  assignmentId,
}: {
  cohortId: string;
  ...
  return (
    <div className="space-y-4">
      <Link
        href={`/cohorts/${cohortId}?tab=assignments`}
